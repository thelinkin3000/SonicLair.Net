...
namespace SonicLair.Cli
{
    public class Headless
    {
        ...
        public async void Configure()
        {
            ...
            var path = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".soniclair", "config.json");
            if (File.Exists(path))
            {
                try
                {
                    using (var configFile = File.OpenText(path))
                    {
                        ...
                        if(a != null && a.Any())
                        {
                            Console.WriteLine($"Connected to {account.Url} as {account.Url} and listening...");
                        }
                        else
                        {
